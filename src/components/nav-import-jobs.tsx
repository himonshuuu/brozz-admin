...
export function NavImportJobs({ limit = 3 }: { limit?: number }) {
  ...
  const cacheKey = useMemo(
    () => `importJobs.cache.${user?.role ?? "guest"}.${user?.id ?? "anonymous"}`,
    [user?.id, user?.role],
  );

  useEffect(() => {
    const cached = safeParseJobs(window.localStorage.getItem(cacheKey));
    if (cached?.length) setJobs(cached);

    let cancelled = false;
    listImportJobs(user?.role === "admin" ? undefined : { orgId: user?.id })
      .then((res) => {
        if (cancelled) return;
        const next = res.data;
        setJobs(next);
